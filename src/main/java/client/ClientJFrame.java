...
public class ClientJFrame {
    ...
    public final synchronized void addToChatLog(String message) {
        ...
        client.getClientLogger().addToChatLog(message + System.lineSeparator());
    }

    public void getInfoMessagePane(String infoMessage) {
        JOptionPane.showMessageDialog(jFrame, infoMessage);
    }

    public void showChatJFrame(Client client, TCPConnection tcpConnection) {
        jFrame.setLayout(new BoxLayout(jFrame.getContentPane(), BoxLayout.Y_AXIS));
        jFrame.setTitle("Простой сетевой чат. Пользователь: " + client.getNickname());
        ...
        inputTextArea.setToolTipText("Введите сообщение для отправки");
    ...
    public String getAuthorizationNickname() {
        ...
        if (nickname == null || nickname.isEmpty()) {
            ...
            int isAgain = JOptionPane.showOptionDialog(jFrame,
                    ...
                    PLAIN_MESSAGE,
                    null, null, null);
            if (isAgain == 0) getAuthorizationNickname();
            return nickname;
        }
        jFrame.dispose();
    ...
    public void showServerResetMessage() {
        jFrame.setVisible(true);
        JOptionPane.showMessageDialog(jFrame, "Ошибка связи с свервером!\n" +
